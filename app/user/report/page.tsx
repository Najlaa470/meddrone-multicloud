...
export default function ReportPage() {
  ...
  const [persons, setPersons] = useState("1"); // string pour l'UI
  ...
  // Convertir le nombre de personnes en valeur numérique approximative
  const computePersonsNumber = (value: string): number => {
    switch (value) {
      case "3_5":
        return 4;
      case "6_10":
  ...
  // -----------------------------
  // 🚀 Envoi de l’incident (Firebase + Azure)
  // -----------------------------
  const sendIncident = async () => {
    ...
    if (!currentUser) {
      setStatus("❌ Vous devez être connecté pour envoyer un incident.");
      return;
    }
    ...
    const incident = {
      ...
      severity,
      zone,
      persons: computePersonsNumber(persons),
      conscious,
      bleeding,
      doctorNearby,
      stadiumAlerted,
    ...
    try {
      ...
      // 1️⃣ Enregistrement dans Firebase (pour ton dashboard admin)
      const firebaseRef = await push(ref(db, "incidents"), incident);
      console.log("✅ Firebase ID:", firebaseRef.key);

      setStatus("🔄 Envoi vers Azure…");

      // 2️⃣ Appel de la fonction Azure (qui enverra vers AWS + Event Grid)
      const res = await fetch(
        "https://fa-urgence-drones-hde5dwf2cdapcyfe.francecentral-01.azurewebsites.net/api/newIncident",
        {
          ...
          body: JSON.stringify({
            ...incident,
            firebaseId: firebaseRef.key, // utile pour corréler côté Azure
          }),
        }
      );
      ...
      if (!res.ok) {
        console.error("❌ Erreur Azure:", data);
        throw new Error((data && (data.error as string)) || `HTTP ${res.status}`);
      }

      console.log("✅ Azure:", data);
      setStatus(
        `✅ Incident envoyé ! ID Azure : ${
          (data && (data.incidentId as string)) || "inconnu"
        }`
      );
      ...
      setStatus("❌ Erreur : " + (e.message || "Impossible d’envoyer l’incident"));
    }
  };
      ...
      <div className="max-w-xl mx-auto p-4 space-y-4">
        ...
        <div className="space-y-4">
          <h2 className="text-base font-semibold">
            Formulaire de signalement avancé
          </h2>
          <p className="text-xs text-slate-400">
            Remplissez les informations ci-dessous pour permettre à l’équipe
            médicale et au drone de réagir le plus efficacement possible.
          </p>
          ...
          <section className="space-y-3 rounded-2xl border border-slate-800 bg-slate-900/70 p-3">
            ...
            <div className="space-y-1">
              ...
              >
                ...
                <option value="perte_connaissance">
                  Perte de connaissance
                </option>
                <option value="detresse_respiratoire">
                ...
                <option value="fumee_incendie">
                  Fumée / début d’incendie
                </option>
